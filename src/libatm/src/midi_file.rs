...
impl MIDIHeader {
    ...
    pub fn new(chunk_type: Vec<u8>, length: u32, format: MIDIFormat, tracks: u16, division: u16) -> Self {
        Self { chunk_type, length, format: format as u16, tracks, division }
    }
...
impl MIDIFile {
    ...
    /// ```
    pub fn new(sequence: crate::midi_note::MIDINoteVec, format: MIDIFormat, tracks: u16, division: u16) -> MIDIFile {
        MIDIFile { sequence, format, tracks, division }
    }
    ...
    pub fn gen_hash(&self) -> String {
        self.sequence.iter().map(|note| note.convert().to_string()).collect::<Vec<String>>().join("")
    }
