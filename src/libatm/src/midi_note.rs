...
impl<'a> std::str::FromStr for MIDINoteType {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        match s.to_lowercase().as_str() {
            "bsharp" | "b#" | "c" => Ok(Self::C),
            "csharp" | "c#" | "dflat" | "d♭" | "db" => Ok(Self::CSharp),
            "d" => Ok(Self::D),
            "dsharp" | "d#" | "eflat" | "e♭" | "eb" => Ok(Self::DSharp),
            "e" | "fflat" | "f♭" | "fb" => Ok(Self::E),
            "esharp" | "e#" | "f" => Ok(Self::F),
            "fsharp" | "f#" | "gflat" | "g♭" | "gb" => Ok(Self::FSharp),
            "g" => Ok(Self::G),
            "gsharp" | "g#" | "aflat" | "a♭" | "ab" => Ok(Self::GSharp),
            "a" => Ok(Self::A),
            "asharp" | "a#" | "bflat" | "b♭" | "bb" => Ok(Self::ASharp),
            "cflat" | "c♭" | "cb" | "b" => Ok(Self::B),
            "rest" | "empty" => Ok(Self::Rest),
            _ => Err(ParseMIDINoteTypeError::UnknownNoteType { input: s.to_string() }),
        }
    }
}
...
impl<'a> std::str::FromStr for MIDINote {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        ...
        if split_pair.len() != 2 {
            return Err(ParseMIDINoteError::InvalidNoteFormat { input: s.to_string() });
        }
...
impl std::str::FromStr for MIDINoteSet {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        let notes = s
            ...
            .enumerate()
            .map(|(idx, pair)| {
                pair.parse::<MIDINote>().map_err(|err| ParseMIDINoteSequenceError::ParseMIDINote(idx, err))
            })
...
mod tests {
    ...
    fn test_midi_note_from_str_invalid_note_type() {
        let observed = "C$:5".parse::<MIDINote>();
        let expected = Err(ParseMIDINoteError::UnknownNoteType(ParseMIDINoteTypeError::UnknownNoteType {
            input: "C$".to_string(),
        }));
        assert_eq!(expected, observed);
    }
    ...
    fn test_midi_note_set_from_str_valid_no_duplicate() {
        ...
        let expected = Ok(MIDINoteSet(
            ...
            .into_iter()
            .collect::<std::collections::BTreeSet<MIDINote>>(),
        ));
        assert_eq!(expected, observed);
    }
    ...
    fn test_midi_note_set_from_str_valid_with_duplicate() {
        ...
        let expected = Ok(MIDINoteSet(
            vec![MIDINote::new(MIDINoteType::C, 4), MIDINote::new(MIDINoteType::D, 5)]
                .into_iter()
                .collect::<std::collections::BTreeSet<MIDINote>>(),
        ));
        assert_eq!(expected, observed);
