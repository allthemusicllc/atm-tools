...
pub enum PartitionPathGeneratorError {
    ...
    #[error(
        "Partition depth must be less than the length of generated melodies \
            ({partition_depth} > {melody_length})"
    )]
    PartitionDepthLongerThanMelody { partition_depth: u32, melody_length: u32 },
    #[error(
        "Melodies of length {melody_length} cannot be partitioned with depth \
...
impl PartitionPathGenerator {
    ...
    pub fn new(
        ...
        if partition_depth > melody_length as u32 {
            return Err(PartitionPathGeneratorError::PartitionDepthLongerThanMelody { partition_depth, melody_length });
        }
        ...
        if !(num_notes == 1 || num_melodies <= max_files.into()) {
            ...
            calc_partition_length =
                Self::gen_partition_length(num_notes, num_melodies, melody_length, max_files, partition_depth)?;
        }
...
impl PathGenerator for PartitionPathGenerator {
    fn gen_path_for_file(&self, mfile: &libatm::MIDIFile) -> Result<String, PathGeneratorError> {
        ...
        Ok(format!("{}", std::path::Path::new(&basename).join(&filename).as_path().to_string_lossy(),))
    }
}
    ...
    macro_rules! check_num_files_partition {
        ($test_name:ident, $note_set:expr, $melody_length:expr, $max_files:expr, $partition_depth:expr) => {
            ...
            fn $test_name() {
                ...
                let path_generator =
                    PartitionPathGenerator::new(num_notes, $melody_length, $max_files, $partition_depth).unwrap();

                for melody in crate::utils::gen_sequences(&Vec::from(&notes), $melody_length) {
                    // Generate partition for melody
                    let melody_partition = path_generator
                        .gen_basename_for_file(&libatm::MIDIFile::new(
