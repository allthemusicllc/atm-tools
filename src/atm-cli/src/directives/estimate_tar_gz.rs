...
use crate::{
    cli::CliDirective,
    directives::{
        estimate::{gen_sim_file_size, gen_sim_num_melodies, pad_value_to_block},
        gen::try_compression_from_str,
    },
    storage::{tar_archive::TarArchive, IntoInner, MIDIHashPathGenerator, StorageBackend},
    utils::{gen_num_melodies, gen_sequences},
};
...
fn estimate_tar_gz_size(
    ...
    let mut archive =
        TarArchive::new(GzEncoder::new(std::io::BufWriter::new(Vec::new()), compression_level), MIDIHashPathGenerator);

    // For each melody
    for (idx, melody_ref) in gen_sequences(notes, melody_length).enumerate() {
        if idx as u64 == num_melodies {
            break;
        }
...
impl CliDirective for EstimateTarGzDirective {
    fn run(self) {
        ...
        println!(
            concat!(
                "Number of distinct notes:               {num_notes}\n",
