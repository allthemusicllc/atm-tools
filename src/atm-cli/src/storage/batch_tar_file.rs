...
use crate::storage::{
    IntoInner, MIDIHashPathGenerator, PartitionPathGenerator, PathGeneratorError, StorageBackend, StorageState,
    TarArchive, TarArchiveError,
};
...
impl BatchTarFile {
    ...
    fn gen_batch_archive(compression_level: Compression) -> TarArchive<BatchInnerObject, MIDIHashPathGenerator> {
        TarArchive::new(
            GzEncoder::new(std::io::BufWriter::new(Vec::with_capacity(512)), compression_level),
    ...
    pub fn new<P: AsRef<std::path::Path>>(
        ...
        let archive = tar::Builder::new(std::io::BufWriter::new(std::fs::File::create(target_path)?));

        // Generate partition manager
        let path_generator = PartitionPathGenerator::new(num_notes, melody_length, max_files, partition_depth)
            .map_err(|e| TarArchiveError::PathGenerator(PathGeneratorError::PartitionPathGenerator(e)))?;
